...
export default function ShopPage() {
  ...
  // <-- URL se query parameter read karna -->
  const searchParams = useSearchParams();
  const categoryId = searchParams.get('category'); // Ye URL se '3' ya jo bhi id ho, nikalega
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        // 1. Fetch Categories (Left Sidebar)
        ...
        // 2. Fetch Products (DYNAMIC API URL)
        // Agar categoryId hai toh us specific category ka data layega, warna saare products layega
        const apiUrl = categoryId 
          ? `/api/categories/${categoryId}` 
          : '/api/admin/products?page=1&limit=48';
        ...
        // Handle dono tarah ke API response formats gracefully
        if (prodResult?.success && Array.isArray(prodResult?.data)) {
          fetchedProd = prodResult.data;
        } else if (prodResult?.data?.data) {
          fetchedProd = prodResult.data.data;
        } else if (prodResult?.products) {
          fetchedProd = prodResult.products;
    ...
    fetchData();
  }, [categoryId]); // Dependency array me categoryId dal diya taaki URL change hone par data wapas fetch ho

  const handleAddToCart = (e, product, primaryImg) => {
    e.preventDefault(); 
    
    // Safely extract color and size
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            ...
            <div className="mb-8 border-b border-gray-100 pb-8">
              ...
              <ul className="flex flex-col gap-3 text-sm text-gray-600">
                <li>
                  <Link href="/shop" className={`hover:text-black transition-colors w-full text-left flex justify-between ${!categoryId ? 'font-bold text-black' : ''}`}>
                    All Collections
                  </Link>
                </li>
                {categories.map((cat) => {
                  ...
                  return (
                    <li key={catId}>
                      {/* Button hata kar Link lagaya taaki URL click hone par update ho */}
                      <Link 
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-10">
                {products.map((product) => {
                  ...
                  const primaryImg = typeof img1 === 'string' ? img1 : (img1?.imageUrl || 'https://placehold.co/600x600/eeeeee/31343c?text=No+Image');
                  const secondaryImg = typeof img2 === 'string' ? img2 : (img2?.imageUrl || primaryImg);
